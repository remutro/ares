...
*/
  ...
  auto initialize() -> bool {
    ...
    if(FAILED(generateSwapChain())) { return false; }

    // Create a render target view
    hr = _pSwapChain->GetBuffer( 0, __uuidof( ID3D11Texture2D ), reinterpret_cast<void**>( &_pTexture2D ) );
    if(FAILED(hr)) { return false; }

    hr = _pd3dDevice->CreateRenderTargetView( _pTexture2D, nullptr, &_pRenderTargetView );
    _pTexture2D->Release();
    if(FAILED(hr)) { return false; }
