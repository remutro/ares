  ...
  */
  ...
  auto output(u32 width, u32 height) -> void override {
    ...
    */
    ...
    u32 y = (_windowHeight - height) / 2;
    //setVertex(0, 0, _inputWidth, _inputHeight, _textureWidth, _textureHeight, x, y, width, height);
    _pd3dDevice->SetTexture(0, _pTexture2D);
    ...
    */
    ...
    // Render a triangle
	_pImmediateContext->VSSetShader( g_pVertexShader, nullptr, 0 );
	_pImmediateContext->PSSetShader( g_pPixelShader, nullptr, 0 );
...
*/
  ...
  auto initialize() -> bool {
    ...
    hr = CompileShaderFromFile( L"shaders.fxh", "VS", "vs_4_0", &pVSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( nullptr,
	...
	hr = _pd3dDevice->CreateVertexShader( pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), nullptr, &g_pVertexShader );
	if( FAILED( hr ) )
	{	
		pVSBlob->Release();
    ...
    hr = CompileShaderFromFile( L"shaders.fxh", "PS", "ps_4_0", &pPSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( nullptr,
                    L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK );
	...
	hr = _pd3dDevice->CreatePixelShader( pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), nullptr, &g_pPixelShader );
	pPSBlob->Release();
    if( FAILED( hr ) ) {
  ...
  auto generateSwapChain() -> HRESULT {
    ...
    if(dxgiFactory2) {
        ...
        hr = dxgiFactory2->CreateSwapChainForHwnd( _pd3dDevice, _context, &sd, nullptr, nullptr, &_pSwapChain1 );
        if(SUCCEEDED(hr)) {
    ...
    dxgiFactory->MakeWindowAssociation( _context, DXGI_MWA_NO_ALT_ENTER );
    dxgiFactory->Release();
